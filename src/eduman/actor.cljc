(ns eduman.actor
  "EducationManagersActor — the ISCO-08 1345 community eduman actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one eduman operation request
  ...
  The unconditional invariant: the EducationManagersAdvisor can never
  directly commit a record the EducationManagersGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [eduman.advisor :as advisor]
            [eduman.governor :as governor]
            [eduman.store :as store]))

(defn build-graph
  "Build a compiled EducationManagersActor graph. `store` implements
  `eduman.store/Store`. `advisor` implements
  `eduman.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :program-id (:program-id proposal)
                                    :payload proposal}]
